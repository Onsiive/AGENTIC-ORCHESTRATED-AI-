...
## v5.0-agentic-evaluation

### Added
- Added `utils/agentic_evaluation.py` as the dedicated Agentic AI evaluation layer.
- Added trajectory logging with thought/action/observation/reflection records.
- Added planning evaluation, tool selection evaluation, multi-agent evaluation, benchmark scoring, governance/safety scoring, robustness testing, and adaptive workflow controller.
- Added agentic evaluation artifacts: JSON report, Markdown report, trajectory JSONL, optional chart PNG.
- Added agentic metrics to side-by-side comparison report.

### Changed
- `LLMPlannerAgent` now evaluates the workflow as an agentic system, not only as an ML pipeline.
- Planner prompt now asks for safe visible reasoning summaries and expected observations/reflections.
- LLM planner now defaults to Qwen local (Ollama) when no llm_agent is provided.
- Offline/mock fallback keeps the workflow runnable without API keys.
- Non-interactive execution automatically approves HITL review for tests/batch mode.

### Preserved
- Existing data loading, preprocessing, model benchmark, recommendation system, and HITL audit remain intact.
