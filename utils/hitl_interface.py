...
class CliHitlInterface(HitlInterface):
    ...
    def show_info(self, message: str, data: Optional[Any] = None):
        """Show information via command line interface in human-readable format."""
        import pandas as pd

        # Skip raw technical headers that are not useful for users
        skip_messages = [
            "===== HUMAN-IN-THE-LOOP REVIEW =====",
            "Detailed Digital Marketing Recommendations:",
        ]
        if message.strip() in skip_messages:
            return

        print(f"\n{message}")

        if data is not None:
            if hasattr(data, 'to_string') and hasattr(data, 'columns'):
                # It's a DataFrame — render each row in a card-like format
                df = data
                LABEL_MAP = {
                    'CustomerID': 'ID Pelanggan',
                    'Predicted_Conversion': 'Prediksi Konversi',
                    'Priority_Level': 'Prioritas',
                    'Decision_Area': 'Area Keputusan',
                    'Recommended_Action': 'Rekomendasi Aksi',
                    'Estimated_Impact': 'Estimasi Dampak',
                    'Timeframe': 'Waktu Pelaksanaan',
                    'CampaignChannel': 'Kanal Kampanye',
                    'CampaignType': 'Tipe Kampanye',
                    'Engagement_Status': 'Status Engagement',
                    'Contributing_Factors': 'Faktor Pendorong',
                    'Reason_for_Action': 'Alasan',
                    'Model_Confidence': 'Kepercayaan Model',
                    'index': 'Nomor Baris',
                }
                PRIORITY_ICON = {
                    'Critical': '🔴',
                ...
                SHOW_COLS = [
                    'index', 'Priority_Level', 'Decision_Area',
                    'Recommended_Action', 'CampaignChannel',
                    'Engagement_Status', 'Timeframe',
                ]
                cols_available = [c for c in SHOW_COLS if c in df.columns]
                ...
                for row_i, (_, row) in enumerate(df.iterrows()):
                    ...
                    print(f"\n  [{idx_val}] {icon} {priority.upper()} · {channel}")
                    print(f"       Area      : {area}")
                    print(f"       Aksi      : {action}")
                    print(f"       Engagement: {engagement}")
                    print(f"       Waktu     : {timeframe}")
                    print("  " + "─" * 68)
