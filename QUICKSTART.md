...
## 🔬 Mode Perbandingan Dua AI (Baru!)

Jalankan Qwen (lokal) dan Gemini (cloud) secara berurutan dan bandingkan hasilnya.

### Setup

**1. Pastikan Ollama + Qwen sudah berjalan:**
```bash
ollama pull qwen3:4b
```
...
- Klik "Create API Key"
- Salin key ke file `.env`:
```
GEMINI_API_KEY=your_key_here
```

**3. Install dependency Gemini (jika belum):**
```bash
...
```bash
# Mode otomatis (tidak ada input interaktif)
python main_comparison.py --dataset data/Digital\ Marketing\ Campaign\ Dataset/digital_marketing_campaign_dataset.csv --auto

# Mode manual (ada HITL review untuk setiap sistem)
python main_comparison.py --dataset path/to/dataset.csv

# Pilih model berbeda
python main_comparison.py --qwen-model qwen3:4b --gemini-model gemini-2.0-flash --auto
```

### Output yang Dihasilkan

Program akan mencetak laporan seperti ini:

```
══════════════════════════════════════════════════════════════════════════
  🤖  AGENTIC AI COMPARISON REPORT — SIDE BY SIDE
══════════════════════════════════════════════════════════════════════════
  System A : System A: Qwen (qwen3:4b) — Local
  System B : System B: Gemini (gemini-2.0-flash) — Cloud

  📊  1. MODEL PERFORMANCE (Conversion Prediction)
  ──────────────────────────────────────────────────────
  Metric                          System A        System B
  ────────────────────────────────────────────────────────
  Accuracy                       88.88%          89.12%
  F1-Score                       86.03%          87.40%
  ROC-AUC                        76.37%          77.82%

  🧠  2. LLM ORCHESTRATOR RELIABILITY
  ──────────────────────────────────────────────────────
  Fallback Used                      2x              0x
  LLM Reliability                 60.00%          100.00%
  ...
```

Dan menyimpan file:
- `logs/comparison_report_<timestamp>.json`
- `logs/comparison_summary_<timestamp>.csv`
